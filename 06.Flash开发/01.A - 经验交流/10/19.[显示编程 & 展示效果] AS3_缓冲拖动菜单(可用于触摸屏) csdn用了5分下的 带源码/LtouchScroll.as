...
package {
	...
	public class LtouchScroll{
		 ...
		 * @param	MaskMC		//遮罩
		 */
		public function LtouchScroll(ScrollMC:MovieClip, MaskMC:MovieClip) {
			
			v_targetMC = ScrollMC ;
			v_maskMC = MaskMC ;
			
			StopPos = v_targetMC.y;			//原始点
			v_maskMC.y = v_targetMC.y;
			v_maskMC.x = v_targetMC.x;
			distance = StopPos;				//让mc出现时在原始点
			

			LimitLeft = v_maskMC.y;											//左边界
			LimitRight = v_maskMC.y - v_targetMC.height + v_maskMC.height;	//右边界
		...
		private function MouseDown(e:MouseEvent) {
			...
			StartPositionX = v_targetMC.stage.mouseY - v_targetMC.y;				//起始位置
		...
		private function onEnterFrames(e:Event) {
			toPositionX=v_targetMC.stage.mouseY;
		...
		private function EnterFrame(e:Event) {
			...
			v_targetMC.y += (distance-v_targetMC.y) * tween;		//核心语句
